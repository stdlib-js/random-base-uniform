...
tape( 'attached to the returned function is the generator name', function test( t ) {
	var uniform = factory();
	t.strictEqual( uniform.NAME, 'uniform', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var uniform = factory();
	t.strictEqual( typeof uniform.PRNG, 'function', 'has property' );

	uniform = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( uniform.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( uniform.seed ), true, 'has property' );
	t.strictEqual( uniform.seed[ 0 ], 12345, 'equal to provided seed' );

	uniform = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( uniform.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var uniform = factory();
	t.strictEqual( typeof uniform.seedLength, 'number', 'has property' );

	uniform = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( uniform.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var uniform = factory();
	t.strictEqual( isUint32Array( uniform.state ), true, 'has property' );

	uniform = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( uniform.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var uniform = factory();
	t.strictEqual( typeof uniform.stateLength, 'number', 'has property' );

	uniform = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( uniform.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var uniform = factory();
	t.strictEqual( typeof uniform.byteLength, 'number', 'has property' );

	uniform = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( uniform.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	uniform = factory();
	t.strictEqual( typeof uniform.toJSON, 'function', 'has method' );

	o = uniform.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, uniform.NAME, 'has property' );
	...
	t.strictEqual( typeof uniform.toJSON, 'function', 'has method' );
	t.strictEqual( uniform.toJSON(), null, 'returns expected value' );
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( uniform(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
